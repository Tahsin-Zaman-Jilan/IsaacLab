...
"""Motion-planning Step 1b: bare Franka robot, LIVE MOUSE-SLIDER teleop (Isaac Lab port of the
PyBullet `mouse_control()` tool), replacing step1_bare_robot_teleop.py's Se3Keyboard control.

Same minimal scene as Step 1 (ground plane, one light, one Franka arm -- no cube, no table, no
obstacles). Instead of reading discrete keyboard deltas, this opens a live omni.ui window with 6
pose sliders (X, Y, Z, Roll, Pitch, Yaw) + 1 grasp slider, mirroring the original PyBullet
mouse_control()'s addUserDebugParameter layout exactly. Every physics step, the sliders are read
and IK-solved as an ABSOLUTE end-effector target pose in the robot's base frame -- unlike Step 1's
relative delta-pose keyboard commands, the sliders always represent a fixed target, not a delta.
...
Controls:
  ...
  - Close the viewer window (or Ctrl+C in the terminal) to exit. On exit, prints the final
    delta-from-start action tuple (translate, rotate, grasp), exactly like the PyBullet tool did
    when you broke out of its loop.

.. code-block:: bash

    ./isaaclab.sh -p scripts/motion_planning/step1_mouse_control_teleop.py --device cuda:1
...
parser = argparse.ArgumentParser(description="Step 1b: bare Franka robot, live mouse-slider teleop.")
...
class MouseControlWindow:
    ...
    step, and can report an IK-relative delta action on exit -- same shape and math as the
    original: TransEE (dx,dy,dz), RotEE (droll,dpitch,dyaw as euler), grasp.
    """
...
def run_mouse_teleop(sim: sim_utils.SimulationContext, scene: InteractiveScene) -> None:
    """Live mouse-slider teleop loop, replacing step1_bare_robot_teleop.py's Se3Keyboard control."""
    robot = scene["robot"]
    ...
    # DifferentialIKController in absolute-pose mode -- the sliders always represent an
    # absolute target, unlike step1's relative delta-pose keyboard commands.
    ...
    # markers to visualize current EE frame and commanded goal frame, same as step1's teleop loop
    frame_marker_cfg = FRAME_MARKER_CFG.copy()
    frame_marker_cfg.markers["frame"].scale = (0.08, 0.08, 0.08)
    ee_marker = VisualizationMarkers(frame_marker_cfg.replace(prim_path="/Visuals/ee_current"))
    goal_marker = VisualizationMarkers(frame_marker_cfg.replace(prim_path="/Visuals/ee_goal"))

    mouse_control = MouseControlWindow(robot, ee_body_idx=ee_body_idx)
    ...
    try:
        while simulation_app.is_running():
            ...
            ee_pos_b, ee_quat_b = mouse_control._get_ee_pose_in_base_frame()
            joint_pos_cur = robot.data.joint_pos[:, robot_entity_cfg.joint_ids]

            joint_pos_des = ik_controller.compute(
                ee_pos_b.unsqueeze(0), ee_quat_b.unsqueeze(0), jacobian, joint_pos_cur
            )
